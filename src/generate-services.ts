...
function generateRegularRpcMethod(ctx: Context, methodDesc: MethodDescriptorProto): Code {
  ...
  let encode = options.outputGenericClientImpl ? code`JSON.stringify(${rawInputType}.toJSON(request))` : code`${rawInputType}.encode(request).finish()`;
  let beforeRequest;
...
export function generateRpcType(ctx: Context, hasStreamingMethods: boolean): Code {
  ...
  if (options.outputGenericClientImpl) {
    methods.push([code`request`, code`string`, code`Promise<string>`]);
  } else {
